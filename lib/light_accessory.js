...
const debounce = require('./custom_debounce');
...
const COLOR_MODES = ['color', 'colour','white'];
...
class LightAccessory extends BaseAccessory {
    ...
    constructor(platform, homebridgeAccessory, deviceConfig) {
        ...
        this._setColor = _.debounce(this.__setColor.bind(this), 500, { leading: false, trailing: true});    
    }
    ...
    supports(characteristic) {
        switch (characteristic) {
            ...
            case Characteristic.ColorTemperature:
                return helpers.objectHasKeys(this.deviceConfig.data, 'color_temp')
            default:
                return false;
        }
    }
    ...
    setSaturation(percentage, callback) {
        this.log.debug('[setSaturation][%s]: %s', this.homebridgeAccessory.displayName, percentage);
    ...
    setHue(hue, callback) {
        ...
        this.log.debug('[SetHue][%s] Characteristic.Hue: %s', this.homebridgeAccessory.displayName, hue);
    ...
    getColorTemperature(callback) {
        if (this.hasValidCache()) {
            callback(null, this.getCachedState(Characteristic.ColorTemperature));
        } else {
            // Retrieve device state from Tuya Web API
            this.platform.tuyaWebApi.getDeviceState(this.deviceId).then((data) => {
                // data.brightness only valid for color_mode!=color > https://github.com/PaulAnnekov/tuyaha/blob/master/tuyaha/devices/light.py
                ...
                this.log.debug('[GET][%s] Characteristic.ColorTemperature: %s\%', this.homebridgeAccessory.displayName, data.color_temp);
                this.updateColorTemperature(data, callback);
            }).catch((error) => {
                this.log.error('[GET][%s] Characteristic.ColorTemperature Error: %s', this.homebridgeAccessory.displayName, error);
                this.invalidateCache();
                callback(error);
            });
        }
    ...
    setColorTemperature(mired, callback) {
        this.log.debug('setColorTemperature mired='+mired);

        const tempK = Math.floor(1000000 / mired); //range => 2000K - 7200K
        const percentage = Math.floor((tempK - 2000)/(5200) * 9000) + 1000;

        this.log.debug('setColorTemperature pct='+percentage);
        
        this.platform.tuyaWebApi.setDeviceState(this.deviceId, 'colorTemperatureSet', {value: percentage}).then(() => {
            this.log.debug('[SET][%s] Characteristic.ColorTemperature: %s', this.homebridgeAccessory.displayName, percentage);
            this.setCachedState(Characteristic.ColorTemperature, mired);
            callback();
        }).catch((error) => {
            this.log.error('[SET][%s] Characteristic.ColorTemperature Error: %s', this.homebridgeAccessory.displayName, error);
            this.invalidateCache();
            callback(error);
        });
    }
    ...
    updateColorTemperature(data, callback) {
        if (data.color_temp) {
            ...
            this.setCharacteristic(Characteristic.ColorTemperature, percentage, !callback)
            callback && callback(null, percentage)
        }
    }

    setColor(color, callback) {
        this._setColor(color, callback);
        callback(); // done anyway
    }
     ...
     * @param callback
     */
    __setColor(_color, callback) {
        var color = {
            brightness: this.getCachedState(Characteristic.Brightness),
            hue: this.getCachedState(Characteristic.Hue),
            saturation: this.getCachedState(Characteristic.Saturation)
        };
        this.log.debug(
